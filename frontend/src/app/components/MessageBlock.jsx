...
import { User, Cpu, Scale, CheckCircle2, AlertCircle } from 'lucide-react';

function SolutionCard({ title, content, score, isWinner }) {
  return (
    <div className={`flex flex-col flex-1 rounded-xl overflow-hidden border ${isWinner ? 'border-snitch-gold shadow-[0_0_15px_rgba(212,175,55,0.15)]' : 'border-snitch-border'} bg-snitch-gray-light`}>
      <div className={`flex items-center justify-between p-4 border-b ${isWinner ? 'border-snitch-gold/30 bg-snitch-gold/5' : 'border-snitch-border bg-snitch-gray'}`}>
        <div className="flex items-center gap-2">
          <Cpu className={`w-5 h-5 ${isWinner ? 'text-snitch-gold' : 'text-gray-400'}`} />
          <h3 className={`font-semibold ${isWinner ? 'text-snitch-gold' : 'text-gray-200'}`}>{title}</h3>
        </div>
        <div className="flex items-center gap-2">
          {isWinner && <span className="text-xs font-bold text-snitch-dark bg-snitch-gold px-2 py-0.5 rounded-full uppercase tracking-wider">Winner</span>}
          <div className={`text-lg font-bold ${isWinner ? 'text-snitch-gold' : 'text-gray-300'}`}>{score}/10</div>
        </div>
      </div>
...
export default function MessageBlock({ data }) {
    ...
    return (
      <div className="w-full max-w-6xl mx-auto py-8 px-4 flex flex-col gap-6 animate-pulse">
        <div className="self-end max-w-3xl bg-snitch-gray border border-snitch-border rounded-2xl rounded-tr-sm p-5 shadow-md">
          <p className="text-gray-200 whitespace-pre-wrap">{data.problem}</p>
        </div>
        <div className="self-start flex items-center gap-3 text-snitch-gold">
          <Cpu className="w-5 h-5 animate-spin-slow" />
          <span className="text-sm font-medium tracking-wide">Generating competing solutions & judging...</span>
        </div>
      </div>
    );
  }
  ...
  const s1Winner = judge.solution_1_score >= judge.solution_2_score;
  const s2Winner = judge.solution_2_score >= judge.solution_1_score;
    ...
    <div className="w-full max-w-7xl mx-auto py-8 px-4 flex flex-col gap-8">
      ...
      <div className="self-end max-w-3xl bg-snitch-gray-light border border-snitch-border rounded-2xl rounded-tr-sm p-6 shadow-md flex gap-4 items-start">
        <div className="flex-1">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-2">Prompt</h4>
          <p className="text-gray-100 whitespace-pre-wrap text-[15px] leading-relaxed">{problem}</p>
        </div>
      ...
      <div className="w-full flex flex-col gap-6">
        <div className="flex items-center gap-3 ml-2">
          ...
          <h3 className="text-lg font-medium text-gray-200 tracking-wide">Arena Evaluation</h3>
        </div>
        ...
        <div className="bg-snitch-darker border border-snitch-border rounded-xl p-6 mt-2 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-sm">
            <div className="flex flex-col gap-2">
              <span className="text-gray-500 font-medium tracking-wide">Alpha Reasoning</span>
            ...
            <div className="flex flex-col gap-2">
              <span className="text-gray-500 font-medium tracking-wide">Beta Reasoning</span>
              <p className="text-gray-300 leading-relaxed bg-snitch-gray p-4 rounded-lg border border-snitch-border/50">
